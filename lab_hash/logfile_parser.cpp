	...
	while (infile.good()) {
		...
		*/
		...
		if(hasVisited(ll.customer, ll.url)){
			// whenVisitedTable[key] = ll.date;//(whenVisitedTable[key] < ll.date) ? ll.date :time_t();// whenVisitedTable[key];
			if(whenVisitedTable[key]<ll.date){whenVisitedTable[key] = ll.date;}

		}
		else {whenVisitedTable[key] = ll.date;}
		// std::cout<<ll.date<<std::endl;
		bool duplicate = false;
		for (size_t i = 0; i < uniqueURLs.size(); i++) {
			if(uniqueURLs[i] == ll.url) duplicate = true;
		}

		if(!duplicate) uniqueURLs.push_back(ll.url);
	}
	infile.close();
}
...
time_t LogfileParser::dateVisited(const string& customer, const string& url) const{
	...
	*/
	...
	// return whenVisitedTable[key];//whenVisitedTable.find(key);
