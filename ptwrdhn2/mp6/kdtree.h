		...
		*/
		...
		private:
			...
			void printTree(int low, int high, std::vector<std::string> & output,
				...
				*/
				int dimension;
				void build(vector< Point<Dim> > & newPoints, int curr_dim, int lo, int hi);
				int partition( vector< Point<Dim> > & newPoints, int lo, int hi, int pivotIndex, int axis);
				void select( vector< Point<Dim> > & newPoints, int lo, int hi, int n, int axis);
				Point<Dim> find_helper(const Point<Dim> & query, int lo, int hi, int axis) const;
				int distance(const Point<Dim> & target, const Point<Dim> & curr) const;
			};
