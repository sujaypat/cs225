...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies){
	...
	TreeNode *tempA = NULL;
	TreeNode *tempB = NULL;
	TreeNode *tempInner = NULL;
	for(int i = 0; i < (int)frequencies.size(); i++){
		singleQueue.push(new TreeNode(frequencies[i]));
	}

	while(singleQueue.size() + mergeQueue.size() != 1){
		tempA = removeSmallest(singleQueue, mergeQueue);
		if(singleQueue.size() + mergeQueue.size() != 0){
			tempB = removeSmallest(singleQueue, mergeQueue);
		}
		tempInner = new TreeNode(tempA->freq.getFrequency() + tempB->freq.getFrequency());
		tempInner->left = tempA;
		tempInner->right = tempB;

		mergeQueue.push(tempInner);
	}
