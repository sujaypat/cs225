...
void SquareMaze::makeMaze(int width, int height){
    ...
    for (int i = 0; i < width; i++){
        for (int j = 0; j < height; j++){
            ...
            pair< pair< int, int > , int> wallDown ( point, DOWN);
...
bool SquareMaze::canTravel(int x, int y, int dir) const{
    ...
    switch (dir){
        ...
        case UP: return !(downWalls[getCellIndex(x, y-1)]);
        case LEFT: return !(rightWalls[getCellIndex(x-1, y)]);
        case DOWN: return !(downWalls[getCellIndex(x, y)]);
        default:
            return false;
    }
}
...
int SquareMaze::getNextCell( int x, int y, int dir ){
    switch(dir){
        case DOWN: return getCellIndex(x, y) + mazeWidth;
